...
- **Realidade intersubjetiva**
    ...
    - **Exemplo 2**: Em 2010, Laszlo comprou duas pizzas por 10 mil Bitcoins (BTC, {{< fa brands btc >}}). Esta foi a primeira transação comercial conhecida envolvendo BTC. Quanto vale um BTC?
:::

```{ojs}
viewof preco_btc = Inputs.text({ 
...
async function obterPrecoBTC() {
    ...
    preco_btc.value = `R$ ${data.bitcoin.brl.toFixed(2)}`;
}

// Chama a função para obter o preço inicial e atualiza a cada 10 segundos
obterPrecoBTC();
setInterval(obterPrecoBTC, 10000);
```
